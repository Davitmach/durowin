...
const handleSpin = async () => {
  if (inputValue < balance) {
    ...
    try {
      ...
      // 🟢 Только сейчас запускаем спин и звуки
      play('start');          // звук запуска
      setSpin(true);          // спин начинается
      setTimeout(() => {
        play('game');         // основной звук вращения
      }, 300);
...
<div className="font-[600] text-[#FFFFFF] text-[16px] text-center">{language=='eng'?'Hold to start/stop auto-spin' :'Держите, чтобы начать/остановить авто-вращение'}</div>
{showResult&&result&& result.result.ton_win > 0 && <div className="flex items-center gap-[8px] w-full justify-center mt-[24px] translate-y-[-10px]"><svg width="24" height="22" viewBox="0 0 24 22" fill="none" xmlns="http://www.w3.org/2000/svg">
